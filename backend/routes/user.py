...
def create_user(request: user_schema.CreateUser, db: Session = Depends(get_db)):
    user = user_model.User(
        name=request.name, birth_date=request.birth_date, gender=request.gender
    )
    db.add(user)
    db.commit()
    db.refresh(user)
...
@router.put("/update/{user_id}", response_model=user_schema.ShowUser)
def update_user(
    user_id, request: user_schema.UpdateUser, db: Session = Depends(get_db)
):
