...
def create_competition(user_id,request: competition_schema.CreateCompetition,db: Session = Depends(get_db)):
    competition = competition_model.Competition(name=request.name, status=request.status, description = request.description, user_id=user_id)
    db.add(competition)
    db.commit()
...
def show_competition(competition_id, db: Session = Depends(get_db)):
    competition = db.query(competition_model.Competition).filter(competition_model.Competition.id == competition_id).first()
    return competition

# Updating Competition values
@router.put("/{competition_id}",response_model=competition_schema.UpdateCompetition)
def update_competition(competition_id,request: competition_schema.UpdateCompetition,db: Session = Depends(get_db)):
    competition = db.query(competition_model.Competition).filter(competition_model.Competition.id == competition_id).first()
...
def delete_competition(competition_id, db: Session = Depends(get_db)):
     competition = db.query(competition_model.Competition.id).filter(competition_model.Competition.id == competition_id)

     if not competition.first():
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail=f"User with id {competition_id} not found")
